...
// Helper function to convert CSS variable to actual color value
function getCSSColorValue(color: string): string {
...
export function Orb({
  ...
  const actualColors: [string, string] = useMemo(() => {
    return [
      getCSSColorValue(colors[0]),
      getCSSColorValue(colors[1])
    ]
  }, [colors])

  return (
    <div className={className ?? "relative h-full w-full"} style={size ? { width: size, height: size } : undefined}>
      <Canvas
